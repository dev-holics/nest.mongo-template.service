...
export class UserService implements IUserService {
	...
	async findAll<T>(
		...
		return this.userRepository.findAll<T>(where, options);
	}
	...
	async findOne<T>(
		...
		return this.userRepository.findOne<T>(where, options);
	}

	async getTotal(where?: Record<string, any>): Promise<number> {
		return this.userRepository.getTotal(where);
	}
	...
	async deleteOne(
		...
		return this.userRepository.deleteOne(where, options);
	}
	...
	async active(_id: string, options?: IDatabaseOptions): Promise<UserDocument> {
		const update: UserActiveDto = {
			isActive: true,
		};
